...
function ValueDeleteModal({ value, isOpen, onClose }) {
  ...
  const saveData = async (e) => {
    ...
    try {
      openInfoAlert();
      const result = await IndicatorService.deleteValue(value?.id);
      closeMe(true);
      ...
      <Slide direction="down" in={isOpen} timeout={500}>
        ...
        >
          <MDBox display="flex" alignItems="center" justifyContent="space-between" p={2}>
            <MDTypography variant="h5">Delete Value</MDTypography>
            ...
            <Grid container>
              <Grid item xs={12} >
                <MDBox p={2} textAlign="center">
                  <MDTypography variant="h5">{"Are you sure you want to delete '" + value?.amount + "' ?"} </MDTypography>
                </MDBox>
              </Grid>
...
export default ValueDeleteModal;